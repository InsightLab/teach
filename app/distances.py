...
def distanciaDTW(tid1, tid2, df): 
    ...
    for j in range(1, len_tj2):
        DTW[0,j] = distEuclidiana(df1['lat'][0], df1['lon'][0], df2['lat'][j], df2['lon'][j]) + DTW[0,j-1]

   
    for i in range(1, len_tj1):
        DTW[i,0] = distEuclidiana(df2['lat'][0], df2['lon'][0], df1['lat'][i], df1['lon'][i]) + DTW[i-1,0]


    for i in range(1, len_tj1):
        for j in range(1, len_tj2):
            cost = distEuclidiana(df1['lat'][i], df1['lon'][i], df2['lat'][j], df2['lon'][j]) 
